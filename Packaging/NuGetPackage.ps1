...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
    Write-Log (Invoke-Command {.\NuGet.exe pack Package.nuspec -Symbol -Verbosity Detailed})
    $ExitCode = 0
}
Else {
    Write-Log (Invoke-Command {.\NuGet.exe pack Package.nuspec -Verbosity Detailed})
    $ExitCode = 0
}
